...
The server is deployed to **Google App Engine** as the `playground-server` service via the `deploy_server` job in [`.github/workflows/cicd.yml`](../.github/workflows/cicd.yml). Pushes to `master` run `cargo fmt` + `cargo clippy`, then `gcloud app deploy server/app.yaml` from a checkout that includes git tags.
