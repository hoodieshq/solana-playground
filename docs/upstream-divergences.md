...
| B2 | Default build server in production is the **Foundation's App Engine server**, never `api.solpg.io`; upstream talks to `api.solpg.io` directly. **Under review (2026-09-08):** #22 also routes production builds through a same-origin `/api/build` proxy (D28), and Sergey asked to drop it -- the Foundation server's `PG_CLIENT_URLS` already admits our domains | Solana's users are served by Solana's infrastructure, not Acheron's (D30); the CORS premise behind the proxy held for `api.solpg.io` only (D28, amended) | D30 kept; D28 overturned pending answer (PR #22) | `settings/server/server.ts` and `default-endpoint.ts` are ours; upstream's `server.ts` setting is a two-line default -- never take it | The deploy target's `PG_CLIENT_URLS` must list every origin we serve from; if the proxy goes, `REACT_APP_SERVER_URL` is the single knob again |
