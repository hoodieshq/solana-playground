...
| B2 | Default build server in production is the **Foundation's App Engine server**, never `api.solpg.io`; upstream talks to `api.solpg.io` directly. **Settled (2026-09-21):** the same-origin `/api/build` proxy (D28) is dropped -- the Foundation server's `PG_CLIENT_URLS` already admits our domains. PR #22 closed; the surviving default-endpoint work continues on HOO-1688 | Solana's users are served by Solana's infrastructure, not Acheron's (D30); the CORS premise behind the proxy held for `api.solpg.io` only (D28, amended) | D30 kept; D28 overturned (PR #22 closed 2026-09-21, reopened as HOO-1688) | `settings/server/server.ts` and `default-endpoint.ts` are ours; upstream's `server.ts` setting is a two-line default -- never take it | The deploy target's `PG_CLIENT_URLS` must list every origin we serve from; if the proxy goes, `REACT_APP_SERVER_URL` is the single knob again |
