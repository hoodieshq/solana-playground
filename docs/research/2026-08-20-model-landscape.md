...
- Существование `gemini-3.7-flash` (строка в 2.1). На 2026-08-20 в docs Google
  новейший Flash — 3.6, страница `latest-model` называет новыми моделями 3.6
  Flash и 3.5 Flash-Lite, 3.7 не упоминается нигде; API на `gemini-2.5-flash`
  отвечает 404 с редиректом тоже на 3.6. Либо 3.7 вышел и docs отстали, либо
  строка неверна — проверить у первоисточника до того, как на неё сошлются.
- Цены `gemini-3.6-flash`: на официальной pricing-странице не нашли, в таблице
  стоит `n/d`.
