...
**Tech Stack:** styled-components v5, the `PgTheme` registry (`client/src/themes/`), Google Fonts via CSS `@import`, Playwright screenshot loop for verification.
...
- `client/public/` is the assets submodule — **never** add or edit files there. Fonts load via `@import` in `client/src/index.css`.
...
**Files:**
- Create: `client/src/themes/solana-v2/index.ts`, `client/src/themes/solana-v2/solana-v2.ts`, `client/src/themes/solana-v2/theme.ts`
- Modify: `client/src/themes/playground/playground.ts` (drop `isDefault`), `client/src/index.css` (font `@import`)
...
Read `client/src/themes/solana/theme.ts` (the old color-swap — the structural template) and skim the defaults section of `client/src/utils/theme/theme.ts` (from `_theme_fonts` onward) to see every overridable key: `components.{button,editor,input,menu,skeleton,tabs,terminal,toast,tooltip,wallet,modal,markdown,topbar,progressbar}`, `views.{bottom,main,sidebar}`, `highlight`, `font.{code,other}`.
...
```bash
git add client/src/themes client/src/index.css
...
**Files:**
- Create: `client/src/views/sidebar/icons/{explorer,build,test,tutorials,programs}.svg` (assistant already has one)
- Modify: each `client/src/views/sidebar/<page>/<page>.ts` (cold, 0–1 commits/yr) to import its SVG; `client/src/app/Panels/Side/Left/Left.tsx` and `SidebarButton.tsx` (warm, 5/yr) for the indicator and hover treatment.
...
```bash
git add client/src/views/sidebar client/src/app/Panels/Side/Left
...
**Files:**
- Modify: `client/src/components/Topbar/Topbar.tsx` (+ its children, 2/yr), `client/src/components/Tabs/` (2/yr), `client/src/app/Panels/Bottom/Bottom.tsx` (3/yr)
...
```bash
git add client/src/components/Topbar client/src/components/Tabs client/src/app/Panels/Bottom
...
**Files:**
- Modify: `client/src/views/sidebar/assistant/Component/{Assistant,Chat,ChatItem,Connect,Plan}.tsx` (ours — zero merge risk)
...
```bash
git add client/src/views/sidebar/assistant
...
**Files:**
- Modify: theme keys first (`views.main.primary.home.*` in `solana-v2/theme.ts`); structural touch-ups in `client/src/components/Editor/Home/` only where the theme cannot reach.
...
```bash
git add client/src/themes/solana-v2 client/src/components/Editor/Home
...
**Files:**
- Modify: `client/src/app/Panels/Side/Right/Right.tsx` (**hot — 17/yr; minimal diff, flagged in commit body**); remaining rough edges in modals/menus/inputs via `solana-v2/theme.ts` overrides.
...
```bash
git add client/src/app/Panels/Side/Right client/src/themes/solana-v2
